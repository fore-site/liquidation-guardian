...
- **Router contract** `0x5af5194b4b0909eb978e3cf1e25333852277f07d` (3963 bytes) executed the transfer internally.
  - calldata selector `0x9aefaff8`; args = (owner `0x88678…`, recipient `0x88678…`, amount `0.001 ETH`, 65-byte sig).
  - top-level `value` is 0 and gas used 74793 because the ETH moves as an internal transfer, not a bare send.
- **Owner wallet** `0x88678C9ae3798Aa77c753C3a6a028FE9FA5f7E3E` is an **EIP-7702 delegated smart account**:
...
curl -s -X POST https://app.keeperhub.com/api/execute/transfer \
  ...
  -d '{"chainId":"11155111","recipientAddress":"0x88678C9ae3798Aa77c753C3a6a028FE9FA5f7E3E","amount":"0.001","simulate":true}'
...
curl -s -X POST https://app.keeperhub.com/api/execute/transfer \
  ...
  -d '{"chainId":"11155111","recipientAddress":"0x88678C9ae3798Aa77c753C3a6a028FE9FA5f7E3E","amount":"0.001"}'
```
