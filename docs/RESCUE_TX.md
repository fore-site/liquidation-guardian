...
> `ANTHROPIC_API_KEY` was not yet set, so the Guardian used its LLM-free sizing to stay protected.
...
> path (Claude choosing repay vs. supply and explaining it) is wired and ready; it needs a fresh
> at-risk position to demo, since HF is now healthy at 1.5027.
