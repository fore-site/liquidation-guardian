...
| 3 | **The money shot — LLM decides** (40s) | `LLM_TIMEOUT_MS=60000 npm run kh -- rescue --dry-run --transport mcp --json` | "HF is below the 1.15 threshold. The configured LLM selects the rescue lever and explains why. Code computes the amount. REST performs the no-broadcast simulation preflight; the MCP tool is reserved for the real execution." Freeze on `provider: llm`, the reasoning, `transport: mcp`, and the clean simulation. |
| 4 | Live rescue (40s) | `LLM_TIMEOUT_MS=60000 npm run kh -- rescue --transport mcp --json` | "The same validated decision now executes through KeeperHub's hosted MCP path. Show the execution ID, transaction link, receipt status, and health factor confirmation." Freeze on the transaction link and execution ID. |
